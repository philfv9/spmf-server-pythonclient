...
class SPMFGui(tk.Tk):
    ...
    def _build_tab_jobs(self):
        ...
        styled_button(tb, "🗑  Delete All Done",
                      ...
                      tooltip="Delete all DONE/FAILED/CANCELLED "
                              "jobs").pack(side="left", padx=(0, 6))
    ...
    def _submit_job(self):
            ...
            try:
                ...
                try:
                    ...
                    if cr.status_code == 200:
                        console_text = cr.json().get(
                            "consoleOutput", "")
                except Exception:
                    pass

                if final_status in ("FAILED", "CANCELLED"):
                    err_msg = poll_data.get("errorMessage",
                                            "unknown error")
                    ...
                    self.after(0, self._job_error_with_console,
                               f"{final_status} after "
                               f"{exec_ms}ms: {err_msg}",
                               console_text)
                    return
        ...
        self._submit_thread = threading.Thread(target=worker,
                                               daemon=True)
    ...
    def _job_done(self, job_id: str, output: str, console: str,
        ...
        self._log_write(
            f"Job {job_id} completed in {exec_ms} ms.", "ok")
